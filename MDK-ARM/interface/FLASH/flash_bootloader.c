...
void Erase_App_Flash(uint8_t slot)
{
    ...
    uint32_t startPage = (startAddr - FLASH_BASE) / pageSize;
    uint32_t appPages = appSize / pageSize;
...
//校验APP区是否有有效固件（检查MSP初始值是否在RAM范围内）
//成功返回1，失败返回0
int Verify_APP_Integrity_Flash(uint32_t appaddr)
{
    ...
    if (reset_handler < FLASH_BASE_ADDR || reset_handler > (FLASH_BASE_ADDR + FLASH_TOTAL_SIZE))
    {
        return 0;
    }
