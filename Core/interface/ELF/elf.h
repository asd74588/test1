...
/* ===== e_ident 索引 ===== */
#define EI_MAG0    0U  /* 魔数第 1 个字节 */
#define EI_MAG1    1U  /* 魔数第 2 个字节 */
#define EI_MAG2    2U  /* 魔数第 3 个字节 */
#define EI_MAG3    3U  /* 魔数第 4 个字节 */
#define EI_CLASS   4U  /* 文件类型：32/64 位 */
#define EI_DATA    5U  /* 字节序 */
#define EI_VERSION 6U  /* ELF 版本 */
#define EI_NIDENT  16U /* e_ident 字段长度 */

#define ELFCLASS32  1U /* 32 位 ELF */
#define ELFDATA2LSB 1U /* 小端序 */

/* ===== e_type ===== */
#define ET_REL  1U /* 可重定位文件 */
#define ET_EXEC 2U /* 可执行文件 */
#define ET_DYN  3U /* 共享对象（PIC） */

#define EV_CURRENT 1U

/* ===== e_machine ===== */
#define EM_ARM 40U /* ARM 架构 */

/* ===== Section header 类型（sh_type） ===== */
#define SHT_NULL          0U
#define SHT_PROGBITS      1U /* 代码、数据、向量表等 */
#define SHT_SYMTAB        2U /* 符号表 */
#define SHT_STRTAB        3U /* 字符串表 */
#define SHT_RELA          4U /* 带显式加数的重定向表 */
#define SHT_NOBITS        8U /* .bss：文件中不占空间 */
#define SHT_REL           9U /* 不带显式加数的重定向表 */
...
#define SHF_WRITE     0x1U /* 可写 */
#define SHF_ALLOC     0x2U /* 需要分配内存（加载时） */
#define SHF_EXECINSTR 0x4U /* 可执行 */

/* ===== 特殊 section 索引（st_shndx） ===== */
#define SHN_UNDEF  0U      /* 未定义符号 */
#define SHN_ABS    0xFFF1U /* 绝对地址符号（不随重定向改变） */
#define SHN_COMMON 0xFFF2U /* 公共符号 */
...
#define R_ARM_THM_MOVT_PREL     50U
...
typedef struct
{
    uint8_t  e_ident[EI_NIDENT]; /* 魔数 + 文件类型标识 */
...
/* REL 重定向条目（8 字节，无显式加数） */
typedef struct
{
    uint32_t r_offset; /* 需要修正的位置（相对目标 section 起始的偏移） */
    uint32_t r_info;   /* 符号索引 + 重定向类型 */
} elf32_rel;

/* RELA 重定向条目（12 字节，含显式加数） */
typedef struct
{
...
/* ===== r_info 拆解宏 ===== */
#define ELF32_R_SYM(info)  ((info) >> 8U)
#define ELF32_R_TYPE(info) ((uint8_t)(info))

/* ===== st_info 拆解宏 ===== */
#define ELF32_ST_BIND(info) ((info) >> 4U)
#define ELF32_ST_TYPE(info) ((info) & 0x0FU)
