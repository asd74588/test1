...
static void at_cmd_handle_text(const char *text)
{
    ...
    if (s_cmd_ctx.state == AT_CMD_SUCCESS || s_cmd_ctx.state == AT_CMD_ERROR ||
    ...
    if (strstr(text, "OK"))
    {
        if (strcmp(s_cmd_ctx.expect, "OK\r\n") == 0 || strcmp(s_cmd_ctx.expect, "OK") == 0 ||
                         ...
                         unsigned int timeout)
{
    ...
    strncpy(
        s_cmd_ctx.expect, expect_reply ? expect_reply : EXPECT_OK, sizeof(s_cmd_ctx.expect) - 1U);
...
int esp8266_thingsboard_request_firmware_chunk(
