...
static int net_cfg_flash_page_valid(uint32_t page_addr, net_cfg_page_t *page)
{
    ...
    if (page->magic != NET_CFG_MAGIC || page->layout_ver != NET_CFG_LAYOUT_VER ||
        page->payload_len != sizeof(page->payload))
    {
        return 0;
    }
...
static void net_cfg_legacy_read_string(uint16_t base, uint16_t words, char *out, uint32_t out_size)
{
    uint32_t i;
    uint8_t  has_data = 0U;
    uint8_t  raw[(NET_CFG_MQTT_HOST_MAX + 1U) > (NET_CFG_ACCESS_TOKEN_MAX + 1U)
                     ? (NET_CFG_MQTT_HOST_MAX + 1U)
                     : (NET_CFG_ACCESS_TOKEN_MAX + 1U)];
...
void net_cfg_load_default(net_cfg_t *cfg)
{
    ...
    net_cfg_copy_string(
        cfg->wifi_password, sizeof(cfg->wifi_password), NET_CFG_WIFI_PASSWORD_DEFAULT);
...
{
    if (cfg == NULL || cfg->wifi_ssid[0] == '\0' || cfg->mqtt_host[0] == '\0' ||
        cfg->mqtt_port == 0U || cfg->access_token[0] == '\0')
    {
        return 0;
    }
